...
Elm.LandscapeCss = Elm.LandscapeCss || {};
Elm.LandscapeCss.make = function (_elm) {
   "use strict";
   _elm.LandscapeCss = _elm.LandscapeCss || {};
   if (_elm.LandscapeCss.values) return _elm.LandscapeCss.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var mediumBorder = $Css.px(3);
   var green = A3($Css.rgb,10,205,10);
   var gray = A3($Css.rgb,200,200,200);
   var red = A3($Css.rgb,355,0,0);
   var blue = A3($Css.rgb,253,145,152);
   var pink = A3($Css.rgb,65,200,169);
   var white = A3($Css.rgb,255,255,255);
   var backgroundImageUrl = function (url) {    return A2($Css.property,"background-image",A2($Basics._op["++"],"url(",A2($Basics._op["++"],url,")")));};
   var beAt = F2(function (myTop,myLeft) {    return _U.list([$Css.position($Css.absolute),$Css.top(myTop),$Css.left(myLeft)]);});
   var veryLeft = $Css.px(0);
   var veryTop = $Css.px(0);
   var messagesWidth = $Css.vw(30);
   var landscapeWidth = $Css.vw(70);
   var fullHeight = $Css.vh(100);
   var landscapeSize = _U.list([$Css.width(landscapeWidth),$Css.height(fullHeight)]);
   var Angry = {ctor: "Angry"};
   var NewsInjectorPane = {ctor: "NewsInjectorPane"};
   var CurrentlyInvisible = {ctor: "CurrentlyInvisible"};
   var CurrentlyVisible = {ctor: "CurrentlyVisible"};
   var Save = {ctor: "Save"};
   var Notice = {ctor: "Notice"};
   var Chunder = {ctor: "Chunder"};
   var WhereAmI = {ctor: "WhereAmI"};
   var css = $Css.stylesheet(_U.list([A2($Css.selector,
                                     "aside",
                                     A2($Basics._op["++"],
                                     A2(beAt,veryTop,landscapeWidth),
                                     _U.list([$Css.backgroundColor(white)
                                             ,$Css.margin($Css.px(0))
                                             ,$Css.padding($Css.px(2))
                                             ,$Css.height(fullHeight)
                                             ,$Css.width(messagesWidth)
                                             ,A3($Css.border3,mediumBorder,$Css.groove,green)
                                             ,$Css.verticalAlign($Css.top)
                                             ,$Css.overflow($Css.scroll)
                                             ,$Css.boxSizing($Css.borderBox)])))
                                     ,$Css$Elements.canvas(A2($Basics._op["++"],
                                     A2(beAt,veryTop,veryLeft),
                                     A2($Basics._op["++"],
                                     landscapeSize,
                                     _U.list([backgroundImageUrl("images/elmscape.png")
                                             ,A2($Css.property,"background-size","100% 100%")
                                             ,A2($Css.property,"transition","1s ease-in-out")]))))
                                     ,A2($Css.selector,
                                     "main",
                                     A2($Basics._op["++"],A2(beAt,veryTop,veryLeft),A2($Basics._op["++"],landscapeSize,_U.list([$Css.overflow($Css.hidden)]))))
                                     ,A2($Css.selector,"output",_U.list([$Css.fontFamily($Css.monospace),$Css.whiteSpace($Css.noWrap)]))
                                     ,A2(F2(function (x,y) {    return A2($Css._op["."],x,y);}),
                                     NewsInjectorPane,
                                     A2($Basics._op["++"],
                                     A2(beAt,$Css.vh(10),$Css.vw(10)),
                                     _U.list([$Css.width($Css.vw(50))
                                             ,$Css.height($Css.vh(80))
                                             ,A2($Css.property,"transition","0.5s ease-in-out")
                                             ,A2($Css.border2,$Css.px(4),$Css.groove)
                                             ,$Css.backgroundColor(gray)
                                             ,$Css.padding($Css.px(10))])))
                                     ,A2($Css.selector,"textarea",_U.list([$Css.width($Css.pct(100)),$Css.height($Css.pct(90))]))
                                     ,A2(F2(function (x,y) {    return A2($Css._op["."],x,y);}),Angry,_U.list([$Css.color(red)]))
                                     ,A2(F2(function (x,y) {    return A2($Css._op["."],x,y);}),
                                     WhereAmI,
                                     _U.list([$Css.padding($Css.em(0.1))
                                             ,A2($Css.border2,$Css.px(3),$Css.inset)
                                             ,$Css.margin($Css.em(0.2))
                                             ,$Css.minWidth($Css.vw(10))]))
                                     ,A2(F2(function (x,y) {    return A2($Css._op["."],x,y);}),Notice,_U.list([$Css.backgroundColor(pink)]))
                                     ,A2(F2(function (x,y) {    return A2($Css._op["."],x,y);}),
                                     Save,
                                     _U.list([$Css.backgroundColor(blue),$Css.fontWeight($Css.bold)]))
                                     ,A2(F2(function (x,y) {    return A2($Css._op["."],x,y);}),
                                     CurrentlyVisible,
                                     _U.list([A2($Css.border2,$Css.px(3),$Css.inset)]))
                                     ,A2(F2(function (x,y) {    return A2($Css._op["."],x,y);}),
                                     CurrentlyInvisible,
                                     _U.list([A2($Css.border2,$Css.px(3),$Css.outset)]))]));
   return _elm.LandscapeCss.values = {_op: _op
                                     ,css: css
                                     ,beAt: beAt
                                     ,WhereAmI: WhereAmI
                                     ,Chunder: Chunder
                                     ...
                                     ,NewsInjectorPane: NewsInjectorPane
                                     ,Angry: Angry};
};
Elm.Clock = Elm.Clock || {};
...
Elm.Messages.make = function (_elm) {
   ...
   if (_elm.Messages.values) return _elm.Messages.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Clock = Elm.Clock.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $LandscapeCss = Elm.LandscapeCss.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Set = Elm.Set.make(_elm),
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var makeComparable = function (_) {    return _.name;};
   var addVisibility = function (_p0) {    return $Set.insert(makeComparable(_p0));};
   var removeVisibility = function (_p1) {    return $Set.remove(makeComparable(_p1));};
   var isVisible = F2(function (visibility,importance) {    return A2($Set.member,makeComparable(importance),visibility);});
   var Message = F3(function (a,b,c) {    return {importance: a,clock: b,say: c};});
   var save = {name: "Save",cssClass: $LandscapeCss.Save};
   var notice = {name: "Notice",cssClass: $LandscapeCss.Notice};
   var chunder = {name: "Chunder",cssClass: $LandscapeCss.Chunder};
   var importances = _U.list([chunder,notice,save]);
   var allVisible = $Set.fromList(A2($List.map,makeComparable,importances));
   var MessageImportance = F2(function (a,b) {    return {name: a,cssClass: b};});
   return _elm.Messages.values = {_op: _op
...
Elm.Landscape = Elm.Landscape || {};
Elm.Landscape.make = function (_elm) {
   "use strict";
   _elm.Landscape = _elm.Landscape || {};
   if (_elm.Landscape.values) return _elm.Landscape.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var InformativeText = F3(function (a,b,c) {    return {text: a,position: b,naturalZoom: c};});
   return _elm.Landscape.values = {_op: _op,InformativeText: InformativeText};
};
...
Elm.Action.make = function (_elm) {
   ...
   $Char = Elm.Char.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Landscape = Elm.Landscape.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   ...
   var InjectTheNews = {ctor: "InjectTheNews"};
   var NewsInjectorReceiveText = function (a) {    return {ctor: "NewsInjectorReceiveText",_0: a};};
   var DiscardNewsInjector = {ctor: "DiscardNewsInjector"};
   var ActivateNewsInjector = {ctor: "ActivateNewsInjector"};
   var Envisiblate = function (a) {    return {ctor: "Envisiblate",_0: a};};
   var Disvisiblate = function (a) {    return {ctor: "Disvisiblate",_0: a};};
   var ZoomOut = function (a) {    return {ctor: "ZoomOut",_0: a};};
   var ZoomIn = function (a) {    return {ctor: "ZoomIn",_0: a};};
   var SaveText = {ctor: "SaveText"};
   var DiscardText = {ctor: "DiscardText"};
   var ReceiveText = function (a) {    return {ctor: "ReceiveText",_0: a};};
   var NewTextInput = function (a) {    return {ctor: "NewTextInput",_0: a};};
   var NoOp = {ctor: "NoOp"};
   var Click = {ctor: "Click"};
   var MouseMove = F2(function (a,b) {    return {ctor: "MouseMove",_0: a,_1: b};});
   var ServerSays = function (a) {    return {ctor: "ServerSays",_0: a};};
   var DoThis = function (a) {    return {ctor: "DoThis",_0: a};};
   return _elm.Action.values = {_op: _op
                               ...
                               ,ZoomIn: ZoomIn
                               ,ZoomOut: ZoomOut
                               ,Disvisiblate: Disvisiblate
                               ,Envisiblate: Envisiblate
                               ,ActivateNewsInjector: ActivateNewsInjector
                               ,DiscardNewsInjector: DiscardNewsInjector
                               ,NewsInjectorReceiveText: NewsInjectorReceiveText
                               ,InjectTheNews: InjectTheNews
                               ,DoThis: DoThis
...
Elm.Landscape = Elm.Landscape || {};
Elm.Landscape.Calculations = Elm.Landscape.Calculations || {};
Elm.Landscape.Calculations.make = function (_elm) {
   "use strict";
   _elm.Landscape = _elm.Landscape || {};
   _elm.Landscape.Calculations = _elm.Landscape.Calculations || {};
   if (_elm.Landscape.Calculations.values) return _elm.Landscape.Calculations.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Landscape = Elm.Landscape.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var findNewPlace = F4(function (howMuchToZoomIn,currentZ,currentCenter,_p0) {
      var _p1 = _p0;
      var _p2 = currentCenter;
      var xCurrentCenter = _p2._0;
      var yCurrentCenter = _p2._1;
      var divideByZoom = function (a) {    return $Basics.round($Basics.toFloat(a) / $Basics.toFloat(currentZ));};
      var newX = divideByZoom(_p1._0) + (xCurrentCenter - divideByZoom(35));
      var newY = divideByZoom(_p1._1) + (yCurrentCenter - divideByZoom(50));
      return {ctor: "_Tuple2",_0: currentZ + howMuchToZoomIn,_1: {ctor: "_Tuple2",_0: newX,_1: newY}};
   });
   var translateText = F3(function (zoom,_p4,_p3) {
      var _p5 = _p4;
      var _p6 = _p3;
      var yMove = _p6._1 - _p5._1;
      var xMove = _p6._0 - _p5._0;
      return A2($Basics._op["++"],
      "translate(",
      A2($Basics._op["++"],$Basics.toString(xMove),A2($Basics._op["++"],"vw,",A2($Basics._op["++"],$Basics.toString(yMove),"vh)"))));
   });
   var transformText = F3(function (zoom,center,pos) {
      var scale = A2($Basics._op["++"],"scale(",A2($Basics._op["++"],$Basics.toString(zoom),")"));
      var translate = A3(translateText,zoom,center,pos);
      return {ctor: "_Tuple2",_0: "transform",_1: A2($Basics._op["++"],scale,A2($Basics._op["++"]," ",translate))};
   });
   var translateFunction = F2(function (zoomLevel,_p7) {
      var _p8 = _p7;
      var yMove = 50 - _p8._1;
      var xMove = 35 - _p8._0;
      return A2($Basics._op["++"],
   ...
   var transform = F2(function (z,center) {
      var scale = A2($Basics._op["++"],"scale(",A2($Basics._op["++"],$Basics.toString(z),")"));
      var translate = A2(translateFunction,z,center);
      return {ctor: "_Tuple2",_0: "transform",_1: A2($Basics._op["++"],scale,A2($Basics._op["++"]," ",translate))};
   });
   return _elm.Landscape.Calculations.values = {_op: _op
                                               ,transformText: transformText
                                               ,transform: transform
                                               ,translateFunction: translateFunction
                                               ,findNewPlace: findNewPlace};
};
Elm.NewsInjector = Elm.NewsInjector || {};
Elm.NewsInjector.make = function (_elm) {
   "use strict";
   _elm.NewsInjector = _elm.NewsInjector || {};
   if (_elm.NewsInjector.values) return _elm.NewsInjector.values;
   var _U = Elm.Native.Utils.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var init = {isAThing: false,contents: "",pretendWeJustReceived: _U.list([]),error: $Maybe.Nothing};
   var NewsInjectorPane = F4(function (a,b,c,d) {    return {isAThing: a,contents: b,pretendWeJustReceived: c,error: d};});
   return _elm.NewsInjector.values = {_op: _op,NewsInjectorPane: NewsInjectorPane,init: init};
};
Elm.Model = Elm.Model || {};
Elm.Model.make = function (_elm) {
   ...
   $Maybe = Elm.Maybe.make(_elm),
   $Messages = Elm.Messages.make(_elm),
   $NewsInjector = Elm.NewsInjector.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Set = Elm.Set.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $String = Elm.String.make(_elm);
   var _op = {};
   var isOrdinaryChar = function (keyCode) {    return _U.cmp(65,keyCode) < 1 && _U.cmp(keyCode,90) < 1;};
   var betterFromCode = function (keyCode) {
      var _p0 = keyCode;
      switch (_p0)
      {case 38: return "up";
         case 40: return "down";
         ...
         default: var _p1 = _p0;
           return isOrdinaryChar(_p1) ? $String.fromChar($Char.fromCode(_p1)) : $Basics.toString(_p1);}
   };
   var printableKeysDown = function (model) {    return A2($String.join,",",$Set.toList(A2($Set.map,betterFromCode,model.keysDown)));};
   var keysPressed = function (world) {    return A2($Set.diff,world.keysDown,world.previousKeysDown);};
   var init = {world: {pointer: {ctor: "_Tuple2",_0: 0,_1: 0},keysDown: $Set.empty,previousKeysDown: $Set.empty}
              ,state: {messages: _U.list([])
                      ,messageVisibility: $Messages.allVisible
                      ,textInput: {isAThing: false,position: {ctor: "_Tuple2",_0: 0,_1: 0},contents: "",id: ""}
                      ,annotations: _U.list([])
                      ,z: 1
                      ,center: {ctor: "_Tuple2",_0: 35,_1: 50}
                      ,newsInjector: $NewsInjector.init}
              ,clock: 1};
   var Model = F3(function (a,b,c) {    return {world: a,state: b,clock: c};});
   var ApplicationState = F7(function (a,b,c,d,e,f,g) {
      return {messages: a,messageVisibility: b,textInput: c,annotations: d,z: e,center: f,newsInjector: g};
   });
   var OutsideWorld = F3(function (a,b,c) {    return {pointer: a,keysDown: b,previousKeysDown: c};});
   return _elm.Model.values = {_op: _op
                              ...
                              ,printableKeysDown: printableKeysDown
                              ,betterFromCode: betterFromCode
                              ,isOrdinaryChar: isOrdinaryChar};
};
Elm.Landscape = Elm.Landscape || {};
Elm.Landscape.Update = Elm.Landscape.Update || {};
Elm.Landscape.Update.make = function (_elm) {
   "use strict";
   _elm.Landscape = _elm.Landscape || {};
   _elm.Landscape.Update = _elm.Landscape.Update || {};
   if (_elm.Landscape.Update.values) return _elm.Landscape.Update.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Action = Elm.Action.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Landscape$Calculations = Elm.Landscape.Calculations.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var downArrowPressed = function (model) {    return A2($Set.member,40,model.keysDown);};
   var upArrowPressed = function (model) {    return A2($Set.member,38,model.keysDown);};
   var howMuchToZoomIn = 1;
   var zoomIn = F2(function (state,click) {
      var _p0 = A4($Landscape$Calculations.findNewPlace,howMuchToZoomIn,state.z,state.center,click);
      var newZ = _p0._0;
      var newCenter = _p0._1;
      return _U.update(state,{z: newZ,center: newCenter});
   });
   var zoomOut = F2(function (state,pointer) {
      var newZoom = A2($Basics.max,1,state.z - howMuchToZoomIn);
      var newCenter = _U.eq(newZoom,1) ? {ctor: "_Tuple2",_0: 35,_1: 50} : state.center;
      return _U.update(state,{z: newZoom,center: newCenter});
   });
   var seeTheWorld = F2(function (news,model) {
      var _p1 = news;
      if (_p1.ctor === "Click") {
            return upArrowPressed(model) ? _U.list([$Action.ZoomIn(model.pointer)]) : downArrowPressed(model) ? _U.list([$Action.ZoomOut(model.pointer)]) : _U.list([]);
         } else {
            return _U.list([]);
         }
   });
   var respondToActions = F2(function (action,model) {
      var _p2 = action;
      switch (_p2.ctor)
      {case "ZoomIn": return A2(zoomIn,model,_p2._0);
         case "ZoomOut": return A2(zoomOut,model,_p2._0);
         default: return model;}
   });
   return _elm.Landscape.Update.values = {_op: _op
                                         ,respondToActions: respondToActions
                                         ,seeTheWorld: seeTheWorld
                                         ,howMuchToZoomIn: howMuchToZoomIn
                                         ,zoomIn: zoomIn
                                         ,zoomOut: zoomOut
...
Elm.Landscape = Elm.Landscape || {};
Elm.Landscape.View = Elm.Landscape.View || {};
Elm.Landscape.View.make = function (_elm) {
   "use strict";
   _elm.Landscape = _elm.Landscape || {};
   _elm.Landscape.View = _elm.Landscape.View || {};
   if (_elm.Landscape.View.values) return _elm.Landscape.View.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Css = Elm.Css.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var pct = function (i) {    return A2($Basics._op["++"],$Basics.toString(i),"%");};
   var draw = F3(function (z,center,annotation) {
      var positioningCss = A2($LandscapeCss.beAt,$Css.vh($Basics.toFloat(50)),$Css.vw($Basics.toFloat(35)));
      var _p0 = annotation.position;
      var x = _p0._0;
      var y = _p0._1;
      return A2($Html.label,
      _U.list([$Html$Attributes.style(A2($Basics._op["++"],
      $Css.asPairs(positioningCss),
      _U.list([A3($Landscape$Calculations.transformText,z,center,annotation.position)])))]),
      _U.list([$Html.text(annotation.text)]));
   });
   var zoomTo = F2(function (zoomLevel,center) {    return $Html$Attributes.style(_U.list([A2($Landscape$Calculations.transform,zoomLevel,center)]));});
   var view = function (state) {
      return A3($Html.node,
      "main",
      _U.list([]),
      A2($Basics._op["++"],
      _U.list([A2($Html.canvas,_U.list([$Html$Attributes.style(_U.list([A2($Landscape$Calculations.transform,state.z,state.center)]))]),_U.list([]))]),
      A2($List.map,A2(draw,state.z,state.center),state.annotations)));
   };
   return _elm.Landscape.View.values = {_op: _op,view: view};
};
Elm.Serialization = Elm.Serialization || {};
Elm.Serialization.make = function (_elm) {
   "use strict";
   _elm.Serialization = _elm.Serialization || {};
   if (_elm.Serialization.values) return _elm.Serialization.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Action = Elm.Action.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var tuple2list = function (_p0) {    var _p1 = _p0;return _U.list([_p1._0,_p1._1]);};
   var encodeInformativeText = function (record) {
      return $Json$Encode.object(_U.list([{ctor: "_Tuple2",_0: "text",_1: $Json$Encode.string(record.text)}
                                         ,{ctor: "_Tuple2",_0: "position",_1: $Json$Encode.list(A2($List.map,$Json$Encode.$int,tuple2list(record.position)))}
                                         ,{ctor: "_Tuple2",_0: "naturalZoom",_1: $Json$Encode.$int(record.naturalZoom)}]));
   };
   var encodeOutgoingNews = function (og) {
      var _p2 = og;
      if (_p2.ctor === "Save") {
            return $Json$Encode.object(_U.list([{ctor: "_Tuple2",_0: "save",_1: encodeInformativeText(_p2._0)}]));
         } else {
            return $Json$Encode.object(_U.list([{ctor: "_Tuple2",_0: "focus",_1: $Json$Encode.string(_p2._0)}]));
         }
   };
   var encodeNews = function (og) {
      var _p3 = og;
      if (_p3.ctor === "ServerSays") {
            return $Json$Encode.object(_U.list([{ctor: "_Tuple2",_0: "serverSays",_1: encodeOutgoingNews(_p3._0)}]));
         } else {
            return $Json$Encode.string("This is not OK");
         }
   };
   var decodeInformativeText = A2($Json$Decode$Extra._op["|:"],
   A2($Json$Decode$Extra._op["|:"],
   A2($Json$Decode$Extra._op["|:"],$Json$Decode.succeed($Landscape.InformativeText),A2($Json$Decode._op[":="],"text",$Json$Decode.string)),
   A2($Json$Decode._op[":="],
   "position",
   A3($Json$Decode.tuple2,F2(function (v0,v1) {    return {ctor: "_Tuple2",_0: v0,_1: v1};}),$Json$Decode.$int,$Json$Decode.$int))),
   A2($Json$Decode._op[":="],"naturalZoom",$Json$Decode.$int));
   var decodeOutgoingNews = A2($Json$Decode$Extra._op["|:"],$Json$Decode.succeed($Action.Save),A2($Json$Decode._op[":="],"save",decodeInformativeText));
   var decodeNews = A2($Json$Decode$Extra._op["|:"],$Json$Decode.succeed($Action.ServerSays),A2($Json$Decode._op[":="],"serverSays",decodeOutgoingNews));
   return _elm.Serialization.values = {_op: _op
                                      ,decodeNews: decodeNews
                                      ,decodeInformativeText: decodeInformativeText
                                      ,decodeOutgoingNews: decodeOutgoingNews
                                      ,encodeOutgoingNews: encodeOutgoingNews
                                      ,encodeNews: encodeNews
                                      ,encodeInformativeText: encodeInformativeText
                                      ,tuple2list: tuple2list};
};
Elm.Messages = Elm.Messages || {};
Elm.Messages.Update = Elm.Messages.Update || {};
Elm.Messages.Update.make = function (_elm) {
   "use strict";
   _elm.Messages = _elm.Messages || {};
   _elm.Messages.Update = _elm.Messages.Update || {};
   if (_elm.Messages.Update.values) return _elm.Messages.Update.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Action = Elm.Action.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Clock = Elm.Clock.make(_elm),
   ...
   $Model = Elm.Model.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Serialization = Elm.Serialization.make(_elm),
   $Set = Elm.Set.make(_elm),
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var xyz = function (model) {    var _p0 = model.world.pointer;var x = _p0._0;var y = _p0._1;return {ctor: "_Tuple3",_0: x,_1: y,_2: model.state.z};};
   var descriptionOfKeys = function (world) {    return $Set.isEmpty(world.keysDown) ? "" : A2($Basics._op["++"],"+",$Model.printableKeysDown(world));};
   var envisiblate = F2(function (imp,model) {    return _U.update(model,{messageVisibility: A2($Messages.addVisibility,imp,model.messageVisibility)});});
   var disvisiblate = F2(function (imp,model) {    return _U.update(model,{messageVisibility: A2($Messages.removeVisibility,imp,model.messageVisibility)});});
   var respondToActions = F2(function (action,state) {
      var _p1 = action;
      switch (_p1.ctor)
      {case "Disvisiblate": return A2(disvisiblate,_p1._0,state);
         case "Envisiblate": return A2(envisiblate,_p1._0,state);
         default: return state;}
   });
   var seeTheWorld = F2(function (world,news) {    return _U.list([]);});
   var addMessages = F2(function (more,model) {    return _U.update(model,{messages: A2($Basics._op["++"],more,model.messages)});});
   var takeNotice = F3(function (clock,message,model) {    return A2(addMessages,_U.list([A3($Messages.Message,$Messages.notice,clock,message)]),model);});
   var spyOnActions = F3(function (clock,action,state) {    var _p2 = action;return A3(takeNotice,clock,$Basics.toString(action),state);});
   var takeChunder = F3(function (clock,message,model) {    return A2(addMessages,_U.list([A3($Messages.Message,$Messages.chunder,clock,message)]),model);});
   var spyOnNews = F4(function (clock,world,news,state) {
      var _p3 = news;
      if (_p3.ctor === "Click") {
            return A3(takeChunder,clock,A2($Basics._op["++"],"Click: ",A2($Basics._op["++"],$Basics.toString(world.pointer),descriptionOfKeys(world))),state);
         } else {
            var notificationsOfKeyPresses = A2($List.map,
            F2(function (x,y) {    return A2($Basics._op["++"],x,y);})("Press: "),
            A2($List.map,$Model.betterFromCode,$Set.toList($Model.keysPressed(world))));
            return A3($List.foldl,takeChunder(clock),state,notificationsOfKeyPresses);
         }
   });
   var takeSave = F3(function (clock,message,model) {
      var json = $Serialization.encodeOutgoingNews(message);
      return A2(addMessages,_U.list([A3($Messages.Message,$Messages.save,clock,A2($Json$Encode.encode,1,json))]),model);
   });
   var spyOnOutgoingNews = F3(function (clock,news,state) {    return A3(takeSave,clock,news,state);});
   return _elm.Messages.Update.values = {_op: _op
                                        ,seeTheWorld: seeTheWorld
                                        ,respondToActions: respondToActions
                                        ,spyOnNews: spyOnNews
                                        ,spyOnOutgoingNews: spyOnOutgoingNews
                                        ,spyOnActions: spyOnActions
                                        ,takeChunder: takeChunder};
};
Elm.Messages = Elm.Messages || {};
Elm.Messages.View = Elm.Messages.View || {};
Elm.Messages.View.make = function (_elm) {
   "use strict";
   _elm.Messages = _elm.Messages || {};
   _elm.Messages.View = _elm.Messages.View || {};
   if (_elm.Messages.View.values) return _elm.Messages.View.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Action = Elm.Action.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   var makeButton = F3(function (address,isVisible,importance) {
      var _p1 = isVisible(importance) ? {ctor: "_Tuple2",_0: $LandscapeCss.CurrentlyVisible,_1: $Action.Disvisiblate} : {ctor: "_Tuple2"
                                                                                                                        ,_0: $LandscapeCss.CurrentlyInvisible
                                                                                                                        ,_1: $Action.Envisiblate};
      var buttonClass = _p1._0;
      var event = _p1._1;
      return A2($Html.button,
      _U.list([$class(_U.list([buttonClass])),A2($Html$Events.onClick,address,event(importance))]),
      _U.list([$Html.text(importance.name)]));
   });
   var visibility = F2(function (address,model) {
      return A2($Html.div,_U.list([]),A2($List.map,A2(makeButton,address,$Messages.isVisible(model.messageVisibility)),$Messages.importances));
   });
   var output = function (whatToSay) {
      return A2($Html.div,_U.list([$class(_U.list([$LandscapeCss.WhereAmI]))]),_U.list([A2($Html.output,_U.list([]),_U.list([$Html.text(whatToSay)]))]));
   };
   var whereAmI = function (model) {
      return A2($Html.div,
      _U.list([]),
      _U.list([output(A2($Basics._op["++"],"clock: ",$Basics.toString(model.clock)))
              ,output(A2($Basics._op["++"],"mouse: ",$Basics.toString(model.world.pointer)))
              ,output(A2($Basics._op["++"],"zoom: ",$Basics.toString(model.state.z)))
              ,output(A2($Basics._op["++"],"keys: ",$Model.printableKeysDown(model.world)))]));
   };
   var oneMessage = function (m) {
      var clockCycle = _U.eq(m.importance,$Messages.save) ? "" : A2($Basics._op["++"],$Basics.toString(m.clock)," | ");
      return A2($Html.li,_U.list([$class(_U.list([m.importance.cssClass]))]),_U.list([$Html.text(A2($Basics._op["++"],clockCycle,m.say))]));
   };
   var messagePane = F2(function (visibility,whatToSay) {
      return A2($Html.ul,
      _U.list([]),
      A2($List.map,
      oneMessage,
      A2($List.filter,function (_p2) {    return A2($Messages.isVisible,visibility,function (_) {    return _.importance;}(_p2));},whatToSay)));
   });
   var view = F2(function (address,model) {
      var state = model.state;
      return A2($Html.aside,_U.list([]),_U.list([whereAmI(model),A2(visibility,address,state),A2(messagePane,state.messageVisibility,state.messages)]));
   });
   return _elm.Messages.View.values = {_op: _op,view: view};
};
Elm.TextInput = Elm.TextInput || {};
Elm.TextInput.Update = Elm.TextInput.Update || {};
Elm.TextInput.Update.make = function (_elm) {
   "use strict";
   _elm.TextInput = _elm.TextInput || {};
   _elm.TextInput.Update = _elm.TextInput.Update || {};
   if (_elm.TextInput.Update.values) return _elm.TextInput.Update.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Action = Elm.Action.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Char = Elm.Char.make(_elm),
   ...
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Messages$Update = Elm.Messages.Update.make(_elm),
   $Model = Elm.Model.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Set = Elm.Set.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $String = Elm.String.make(_elm);
   var _op = {};
   var theyPushedEnter = function (world) {    return A2($Set.member,13,$Model.keysPressed(world));};
   var theyPushedEscape = function (world) {    return A2($Set.member,27,$Model.keysPressed(world));};
   var theyAreHoldingT = function (world) {    return A2($Set.member,_U.chr("T"),A2($Set.map,$Char.fromCode,world.keysDown));};
   var goodbyeInput = function (model) {
      return _U.update(model,{textInput: {isAThing: false,contents: "",position: {ctor: "_Tuple2",_0: 0,_1: 0},id: ""}});
   };
   var initializeNewInput = F3(function (id,model,position) {
      return _U.update(model,{textInput: {isAThing: true,contents: "",position: position,id: id}});
   });
   var moveItOverABit = function (_p0) {    var _p1 = _p0;return {ctor: "_Tuple2",_0: _p1._0,_1: _p1._1 - 2};};
   var theyHaveEnteredText = function (model) {    return model.textInput.isAThing;};
   var saveTheText = function (state) {
      var textInput = state.textInput;
      var annotation = A3($Landscape.InformativeText,textInput.contents,moveItOverABit(textInput.position),state.z);
      var newState = goodbyeInput(_U.update(state,{annotations: A2($Basics._op["++"],state.annotations,_U.list([annotation]))}));
      return {ctor: "_Tuple2",_0: newState,_1: _U.list([$Action.Save(annotation)])};
   };
   var bePicky = function (input) {
      var removeOrlds = A2($String.split,"orld",input);
      var _p2 = removeOrlds;
      if (_p2.ctor === "::" && _p2._1.ctor === "[]") {
            return {ctor: "_Tuple2",_0: _p2._0,_1: _U.list([])};
         } else {
            return {ctor: "_Tuple2",_0: A2($String.join,"rold",removeOrlds),_1: _U.list(["Fixed that for you ;-)"])};
         }
   };
   var withTextInputContents = F2(function (contents,state) {
      var ti = state.textInput;
      return _U.update(state,{textInput: _U.update(ti,{contents: contents})});
   });
   var doNothing = function (state) {    return {ctor: "_Tuple2",_0: state,_1: _U.list([])};};
   var respondToActions = F3(function (clock,action,state) {
      var _p3 = action;
      switch (_p3.ctor)
      {case "NewTextInput": var newInputId = A2($Basics._op["++"],"textInput_",$Basics.toString(clock));
           return {ctor: "_Tuple2",_0: A3(initializeNewInput,newInputId,state,_p3._0),_1: _U.list([$Action.Focus(newInputId)])};
         case "ReceiveText": var _p4 = bePicky(_p3._0);
           var correctedText = _p4._0;
           var corrections = _p4._1;
           var stateWithMessages = A3($List.foldl,$Messages$Update.takeChunder(clock),state,corrections);
           return doNothing(A2(withTextInputContents,correctedText,stateWithMessages));
         case "DiscardText": return doNothing(goodbyeInput(state));
         case "SaveText": return theyHaveEnteredText(state) ? saveTheText(state) : doNothing(state);
         default: return doNothing(state);}
   });
   var seeTheWorld = F2(function (news,world) {
      var _p5 = news;
      _v3_2: do {
         switch (_p5.ctor)
         {case "ServerSays": if (_p5._0.ctor === "Save") {
                    var _p6 = _p5._0._0;
                    return _U.list([$Action.NewTextInput(_p6.position),$Action.ReceiveText(_p6.text),$Action.SaveText]);
                 } else {
                    break _v3_2;
                 }
            case "Click": return theyAreHoldingT(world) ? _U.list([$Action.NewTextInput(world.pointer)]) : _U.list([]);
            default: break _v3_2;}
      } while (false);
      return theyPushedEscape(world) ? _U.list([$Action.DiscardText]) : theyPushedEnter(world) ? _U.list([$Action.SaveText]) : _U.list([]);
   });
   return _elm.TextInput.Update.values = {_op: _op,seeTheWorld: seeTheWorld,respondToActions: respondToActions};
};
Elm.TextInput = Elm.TextInput || {};
Elm.TextInput.View = Elm.TextInput.View || {};
Elm.TextInput.View.make = function (_elm) {
   "use strict";
   _elm.TextInput = _elm.TextInput || {};
   _elm.TextInput.View = _elm.TextInput.View || {};
   if (_elm.TextInput.View.values) return _elm.TextInput.View.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Action = Elm.Action.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   $String = Elm.String.make(_elm);
   var _op = {};
   var pct = function (i) {    return A2($Basics._op["++"],$Basics.toString(i),"vh");};
   var input = F2(function (address,textInputModel) {
      var _p0 = textInputModel.position;
      var x = _p0._0;
      var y = _p0._1;
      return A2($Html.input,
      _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "position",_1: "relative"}
                                              ,{ctor: "_Tuple2",_0: "top",_1: A2($Basics._op["++"],$Basics.toString(y),"vh")}
                                              ,{ctor: "_Tuple2",_0: "left",_1: A2($Basics._op["++"],$Basics.toString(x),"vw")}
                                              ,{ctor: "_Tuple2"
                                               ,_0: "width"
                                               ,_1: A2($Basics._op["++"],$Basics.toString(8 * $String.length(textInputModel.contents)),"px")}]))
              ,$Html$Attributes.id(textInputModel.id)
              ,$Html$Attributes.value(textInputModel.contents)
              ,A3($Html$Events.on,"input",$Html$Events.targetValue,function (_p1) {    return A2($Signal.message,address,$Action.ReceiveText(_p1));})]),
      _U.list([]));
   });
   var view = F2(function (address,model) {    return model.textInput.isAThing ? _U.list([A2(input,address,model.textInput)]) : _U.list([]);});
   return _elm.TextInput.View.values = {_op: _op,view: view};
};
Elm.NewsInjector = Elm.NewsInjector || {};
Elm.NewsInjector.Update = Elm.NewsInjector.Update || {};
Elm.NewsInjector.Update.make = function (_elm) {
   "use strict";
   _elm.NewsInjector = _elm.NewsInjector || {};
   _elm.NewsInjector.Update = _elm.NewsInjector.Update || {};
   if (_elm.NewsInjector.Update.values) return _elm.NewsInjector.Update.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Action = Elm.Action.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Char = Elm.Char.make(_elm),
   ...
   $String = Elm.String.make(_elm);
   var _op = {};
   var theyAreHoldingCommand = function (world) {    return A2($Set.member,91,world.keysDown) || A2($Set.member,93,world.keysDown);};
   var theyAreHoldingN = function (world) {    return A2($Set.member,_U.chr("N"),A2($Set.map,$Char.fromCode,world.keysDown));};
   var theyPushedEnter = function (world) {    return A2($Set.member,13,$Model.keysPressed(world));};
   var theyPushedEscape = function (world) {    return A2($Set.member,27,$Model.keysPressed(world));};
   var allOrNothing = function () {
      var incorporateOne = F2(function (result,accum) {
         var _p0 = {ctor: "_Tuple2",_0: accum,_1: result};
         if (_p0._0.ctor === "Err") {
               ...
               if (_p0._1.ctor === "Err") {
                     ...
                     return $Result.Ok(A2($Basics._op["++"],_p0._0._0,_U.list([_p0._1._0])));
                  }
            }
      });
      return A2($List.foldl,incorporateOne,$Result.Ok(_U.list([])));
   }();
   var putOriginalInError = F2(function (decode,string) {
      var _p1 = decode(string);
      if (_p1.ctor === "Ok") {
            return $Result.Ok(_p1._0);
         } else {
            return $Result.Err(A2($Basics._op["++"],_p1._0,A2($Basics._op["++"]," <",A2($Basics._op["++"],string,">"))));
         }
   });
   var respondToActions = F2(function (action,state) {
      var newsInjector = state.newsInjector;
      var _p2 = action;
      switch (_p2.ctor)
      {case "ActivateNewsInjector": return _U.update(state,{newsInjector: _U.update(newsInjector,{isAThing: true})});
         case "DiscardNewsInjector": return _U.update(state,{newsInjector: $NewsInjector.init});
         case "NewsInjectorReceiveText": return _U.update(state,{newsInjector: _U.update(newsInjector,{contents: _p2._0})});
         case "InjectTheNews": var lines = A2($String.split,"\n",newsInjector.contents);
           var listOfResults = A2($List.map,putOriginalInError($Json$Decode.decodeString($Serialization.decodeOutgoingNews)),lines);
           var resultOfList = allOrNothing(listOfResults);
           var _p3 = resultOfList;
           if (_p3.ctor === "Ok") {
                 return _U.update(state,
                 {newsInjector: {isAThing: false,contents: "",pretendWeJustReceived: A2($List.map,$Action.ServerSays,_p3._0),error: $Maybe.Nothing}});
              } else {
                 return _U.update(state,{newsInjector: _U.update(newsInjector,{error: $Maybe.Just(_p3._0)})});
              }
         default: return state;}
   });
   var seeTheWorld = F2(function (news,world) {
      var _p4 = news;
      if (_p4.ctor === "Click") {
            return theyAreHoldingN(world) ? _U.list([$Action.ActivateNewsInjector]) : _U.list([]);
         } else {
            return theyPushedEscape(world) ? _U.list([$Action.DiscardNewsInjector]) : theyPushedEnter(world) && theyAreHoldingCommand(world) ? _U.list([$Action.InjectTheNews]) : _U.list([]);
         }
   });
   var pretendThingsHappened = F2(function (model,news) {
      var state = model.state;
      var newsInjector = model.state.newsInjector;
      var moreNews = newsInjector.pretendWeJustReceived;
      var stateWithoutNews = _U.update(state,{newsInjector: _U.update(newsInjector,{pretendWeJustReceived: _U.list([])})});
      return {ctor: "_Tuple2",_0: _U.update(model,{state: stateWithoutNews}),_1: A2($Basics._op["++"],moreNews,_U.list([news]))};
   });
   return _elm.NewsInjector.Update.values = {_op: _op,seeTheWorld: seeTheWorld,respondToActions: respondToActions,pretendThingsHappened: pretendThingsHappened};
};
Elm.NewsInjector = Elm.NewsInjector || {};
Elm.NewsInjector.View = Elm.NewsInjector.View || {};
Elm.NewsInjector.View.make = function (_elm) {
   "use strict";
   _elm.NewsInjector = _elm.NewsInjector || {};
   _elm.NewsInjector.View = _elm.NewsInjector.View || {};
   if (_elm.NewsInjector.View.values) return _elm.NewsInjector.View.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Action = Elm.Action.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var $class = function (_) {    return _.$class;}($Html$CssHelpers.namespace(""));
   var view = F2(function (address,state) {
      var _p0 = function () {
         var _p1 = state.newsInjector.error;
         if (_p1.ctor === "Just") {
               return {ctor: "_Tuple2",_0: _p1._0,_1: _U.list([$class(_U.list([$LandscapeCss.Angry]))])};
            } else {
               return {ctor: "_Tuple2",_0: "paste some news and hit command-enter",_1: _U.list([])};
            }
      }();
      var text = _p0._0;
      var attributes = _p0._1;
      var zoom = state.newsInjector.isAThing ? 1.0 : 0.0;
      return _U.list([A2($Html.div,
      _U.list([$class(_U.list([$LandscapeCss.NewsInjectorPane]))
              ,$Html$Attributes.style(_U.list([{ctor: "_Tuple2"
                                               ,_0: "transform"
                                               ,_1: A2($Basics._op["++"],"scale(",A2($Basics._op["++"],$Basics.toString(zoom),")"))}]))]),
      _U.list([A2($Html.textarea,
              _U.list([A3($Html$Events.on,
                      "input",
                      $Html$Events.targetValue,
                      function (_p2) {
                         return A2($Signal.message,address,$Action.NewsInjectorReceiveText(_p2));
                      })
                      ,$Html$Attributes.value(state.newsInjector.contents)]),
              _U.list([]))
              ,A2($Html.label,attributes,_U.list([$Html.text(text)]))]))]);
   });
   return _elm.NewsInjector.View.values = {_op: _op,view: view};
};
Elm.Main = Elm.Main || {};
Elm.Main.make = function (_elm) {
   ...
   $Landscape$Update = Elm.Landscape.Update.make(_elm),
   $Landscape$View = Elm.Landscape.View.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Messages$Update = Elm.Messages.Update.make(_elm),
   $Messages$View = Elm.Messages.View.make(_elm),
   $Model = Elm.Model.make(_elm),
   $Mouse = Elm.Mouse.make(_elm),
   $NewsInjector$Update = Elm.NewsInjector.Update.make(_elm),
   $NewsInjector$View = Elm.NewsInjector.View.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Serialization = Elm.Serialization.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $TextInput$Update = Elm.TextInput.Update.make(_elm),
   $TextInput$View = Elm.TextInput.View.make(_elm),
   ...
   var view = F2(function (address,model) {
      ...
      _U.list([]),
      A2($Basics._op["++"],
      _U.list([$Landscape$View.view(model.state)]),
      A2($Basics._op["++"],
      _U.list([A2($Messages$View.view,address,model)]),
      A2($Basics._op["++"],A2($TextInput$View.view,address,model.state),A2($NewsInjector$View.view,address,model.state)))));
   });
   var retainOutsideWorld = F2(function (news,model) {
      var _p0 = news;
      if (_p0.ctor === "MouseMove") {
            var before = model.keysDown;
            return _U.update(model,{pointer: _p0._0,keysDown: _p0._1,previousKeysDown: before});
         } else {
            return model;
         }
   });
   var explicitActions = function (news) {    var _p1 = news;if (_p1.ctor === "DoThis") {    return _U.list([_p1._0]);} else {    return _U.list([]);}};
   var respondToNews = F2(function (news,world) {
      return A2($Basics._op["++"],
      A2($Landscape$Update.seeTheWorld,news,world),
      A2($Basics._op["++"],
      A2($TextInput$Update.seeTheWorld,news,world),
      A2($Basics._op["++"],
      A2($Messages$Update.seeTheWorld,news,world),
      A2($Basics._op["++"],A2($NewsInjector$Update.seeTheWorld,news,world),explicitActions(news)))));
   });
   var updateOneSumAnother = F2(function (f,_p2) {
      var _p3 = _p2;
      var _p4 = f(_p3._0);
      var newOne = _p4._0;
      var moreOthers = _p4._1;
      return {ctor: "_Tuple2",_0: newOne,_1: A2($Basics._op["++"],_p3._1,moreOthers)};
   });
   var dropTheSecondInput = F3(function (f,a,_p5) {    var _p6 = _p5;return A2(f,a,_p6._0);});
   var appendSecondOutput = F2(function (f,a) {    return updateOneSumAnother(f(a));});
   var updateOneIgnoreAnother = F2(function (f,_p7) {    var _p8 = _p7;return {ctor: "_Tuple2",_0: f(_p8._0),_1: _p8._1};});
   var respondToAction = F3(function (clock,action,_p9) {
      var _p10 = _p9;
      return A2(updateOneIgnoreAnother,
      $Messages$Update.respondToActions(action),
      A2(updateOneIgnoreAnother,
      $NewsInjector$Update.respondToActions(action),
      A2(updateOneIgnoreAnother,
      $Landscape$Update.respondToActions(action),
      A2(updateOneSumAnother,A2($TextInput$Update.respondToActions,clock,action),{ctor: "_Tuple2",_0: _p10._0,_1: _p10._1}))));
   });
   var respondToOneNews = F2(function (news,model) {
      var world = A2(retainOutsideWorld,news,model.world);
      var spiedOnNews = A4($Messages$Update.spyOnNews,model.clock,world,news,model.state);
      var actions = A2(respondToNews,news,world);
      var spiedState = A3($List.foldl,$Messages$Update.spyOnActions(model.clock),spiedOnNews,actions);
      var _p11 = A3($List.foldl,respondToAction(model.clock),{ctor: "_Tuple2",_0: spiedState,_1: _U.list([])},actions);
      var actedState = _p11._0;
      var outgoingNews = _p11._1;
      var finalState = A3($List.foldl,$Messages$Update.spyOnOutgoingNews(model.clock),actedState,outgoingNews);
      return {ctor: "_Tuple2",_0: {world: world,state: finalState,clock: model.clock + 1},_1: outgoingNews};
   });
   var updateModel = F2(function (news,model) {
      var _p12 = A2($NewsInjector$Update.pretendThingsHappened,model,news);
      var model = _p12._0;
      var allTheNews = _p12._1;
      return A3($List.foldl,appendSecondOutput(respondToOneNews),{ctor: "_Tuple2",_0: model,_1: _U.list([])},allTheNews);
   });
   var mouseClicks = A2($Signal.map,$Basics.always($Action.Click),$Mouse.clicks);
   var relativize = F3(function (_p13,w,h) {    var _p14 = _p13;return {ctor: "_Tuple2",_0: _p14._0 * 100 / w | 0,_1: _p14._1 * 100 / h | 0};});
   var relativeMousePosition = A4($Signal.map3,relativize,$Mouse.position,$Window.width,$Window.height);
   var mousePointer = A3($Signal.map2,$Action.MouseMove,relativeMousePosition,$Keyboard.keysDown);
   var newsFromTheView = $Signal.mailbox($Action.NoOp);
   var app = function () {
      var input = $Signal.mergeMany(_U.list([mousePointer,mouseClicks,newsFromTheView.signal]));
      var modelAndNews = A3($Signal.foldp,dropTheSecondInput(updateModel),{ctor: "_Tuple2",_0: $Model.init,_1: _U.list([])},input);
      return {modelSignal: A2($Signal.map,$Basics.fst,modelAndNews),outgoingNews: A2($Signal.map,$Basics.snd,modelAndNews)};
   }();
   var main = A2($Signal.map,view(A2($Signal.forwardTo,newsFromTheView.address,$Action.DoThis)),app.modelSignal);
   var out = Elm.Native.Port.make(_elm).outboundSignal("out",
   function (v) {
      return Elm.Native.List.make(_elm).toArray(v).map(function (v) {    return v;});
   },
   A2($Signal.map,$List.map($Serialization.encodeOutgoingNews),app.outgoingNews));
